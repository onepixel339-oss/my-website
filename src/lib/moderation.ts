 ...
 * the message is routed to `pending_review` (human review). We NEVER
 * auto-publish when moderation cannot run.
 *
...
export async function moderateMessage(content: string): Promise<ModerationResult> {
  ...
  try {
    ...
    // Classifier unavailable (network, auth, rate-limit, etc.).
    // NEVER auto-publish unmoderated content. Route to human review.
    scores = normalizeScores(null);
    degraded = true;
  }
  ...
  // When the classifier itself failed, override a "publish" decision to
  // "pending_review" so a human always checks unmoderated content.
